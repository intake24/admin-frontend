...
function directiveFun(
  ...
  function controller(scope, element, attributes) {
    ...
    scope.recaptchaEnabled = $window.recaptcha.enabled;

    scope.getBackdropVisibe = function() {
      return (
        scope.modalLogOutVisible ||
        scope.modalAuthenticateVisible ||
        scope.modalResetPasswordVisible
      );
    };

    scope.login = function() {
      UserRequestService.login(scope.username, scope.password).then(function(
        data
      ) {
        UserStateService.init(scope.username, data.refreshToken);
        UserRequestService.refresh()
          .then(
            function(data) {
    ...
    scope.hidePasswordResetModal = function() {
      ...
      if (scope.recaptchaEnabled) $window.grecaptcha.reset();
    ...
    scope.requestPasswordReset = function(r) {
      if (!scope.recaptchaEnabled || scope.recaptchaResponse) {
    ...
    scope.$watchCollection(
      function() {
        return [
          ModalService.getModalAuthenticateVisible(),
